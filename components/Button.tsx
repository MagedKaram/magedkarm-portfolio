...
const LoadingSpinner = ({ icon }: { icon?: boolean }) => (
...
const Button = ({
    ...
    const variantClasses = {
        ...
        light: 'bg-background-light text-foreground hover:bg-background-light',
        ...
        if (props.target === '_blank') {
            return (
                ...
                >
                    ...
                    <span className="z-[1]">
                        {loading ? <LoadingSpinner icon={icon} /> : children}
                    </span>
                </a>
            );
        }
    ...
    const btnProps = rest as ButtonProps;
    return (
        <button className={buttonClasses} {...btnProps}>
            {variant !== 'link' && (
                <span className="absolute top-[200%] left-0 right-0 h-full bg-white rounded-[50%] group-hover:top-0 transition-all duration-500 scale-150"></span>
            )}
            <span className="z-[1]">
